...
func _ready() -> void:
	...
	max_value = (get_viewport_rect().size.y / $"../../Upgrades".available_upgrades.size() * 90) + 20
	get_window().size_changed.connect(showBar)
	
	

func _on_VScrollBar_value_changed (value_changed):
	max_value = (20 + ($"../../Upgrades".available_upgrades.size() - 1)  * 90)
	var i = 0
	for child in $"../../Upgrades".buttons:
		child.position = Vector2(210, (20 + i * 90) - value_changed )
		i+= 1

func showBar():
	custom_minimum_size.y = get_viewport_rect().size.y
	max_value = (get_viewport_rect().size.y / $"../../Upgrades".available_upgrades.size() * 90) + 20
	if (get_viewport_rect().size.y <= 20 + ($"../../Upgrades".available_upgrades.size() * 90)) && $"../../Upgrades".inInv:
		visible = true
	else:
		visible = false
