...
func _on_VScrollBar_value_changed(value):
	...
	max_value = (ceil((($"../Inventory_Button".chosenScreen.size() - 1)/$"../Inventory_Button".xAmount)) - ($"../Inventory_Button".yAmount - 1) - 0.7)
	count = value
	for i in range($"../Inventory_Button".chosenScreen.size()):
		var card = $"../Inventory_Button".chosenScreen[i]
	...
	if $"../Inventory_Button".chosenScreen.size() <= $"../Inventory_Button".xAmount * $"../Inventory_Button".yAmount:
		self.visible = false
